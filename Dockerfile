...
RUN git clone -b $version https://github.com/EOSIO/eos.git --recursive \
    ...
    && cmake -j 2 -DCMAKE_BUILD_TYPE=Release -DBINARYEN_BIN=/eos/externals/binaryen/bin -DWASM_ROOT=/opt/wasm -DOPENSSL_ROOT_DIR=/usr/bin/openssl -DOPENSSL_LIBRARIES=/usr/lib/ssl -DBUILD_MONGO_DB_PLUGIN=true . \
    && make -j 2 \
    && make install
